...
2. Register the client's intake: business type, target public function,
   prior filing history in Georgia if any.
3. Run the advisor in read-only mode against the Unified Electronic
   System of State Procurement (Ge-GP).
4. Compare the checklist against the client's current documentation
   (NAPR registration status).
...
- named referral relationship with Georgian-licensed counsel or a
...
a working referral relationship with Georgian-licensed counsel or a
registered agent for whatever licensed representation the law of
Georgia requires for actual public-procurement filings.
