  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kartuli Fabrication LLC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kartuli Fabrication LLC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest black-list-listed-is-held-and-unoverridable
  (testing "Black List membership -> HARD hold (standard categorical bar, Law of Georgia on Public Procurement Article 3(1)(l))"
    (let [[db actor] (fresh)
      ...
      (is (some #{:black-list-listed} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest white-list-claim-unverified-is-held-and-unoverridable
  (testing "FLAGSHIP: claimed White List simplified-procedure benefit that has NOT been independently verified -> HARD hold (Law of Georgia on Public Procurement Article 3(1)(l1))"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:white-list-claim-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest white-list-claim-verified-proceeds-normally
  (testing "a claimed AND independently-verified White List benefit is NOT held by the flagship check -- still escalates for the usual actuation gate, then commits"
    (let [[db actor] (fresh)
          _ (assess! actor "t7pre" "eng-7")
          _ (draft! actor "t7pre" "eng-7")
          r1 (exec-op actor "t7" {:op :filing/submit :subject "eng-7"} operator)]
...
(deftest id-number-unverified-is-held-and-unoverridable
  (testing "unverified NAPR identification number when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:id-number-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
