(ns marketentry.marketentryllm
  ...
  the Georgia (GEO) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "id-number-verified?=" (:id-number-verified? e)
                        " on-black-list?=" (:on-black-list? e)
                        " claims-white-list-benefit?=" (:claims-white-list-benefit? e)
                        " white-list-verified?=" (:white-list-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-id-number? e))
                              (:id-number-verified? e)))
                   0.9 0.3)}))
