(ns marketentry.store
  "SSoT for the Georgia (GEO) market-entry compliance actor, behind a
  ...
  `:on-black-list?` / `:claims-white-list-benefit?` /
  `:white-list-verified?` / `:requires-id-number?` / `:id-number-verified?`
  are ground truth for the governor's checks -- see
  `marketentry.governor`. `:claims-white-list-benefit?` +
  `:white-list-verified?` together ground the FLAGSHIP check
  (`white-list-claim-unverified`): does the engagement's own proposal
  RELY ON the Law of Georgia on Public Procurement Article 3(1)(l1)
  White List simplified-procedure benefit, and if so, has that
  White-List membership actually been independently verified?
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kartuli Fabrication LLC" :portal "SPA eProcurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-black-list? false
             :claims-white-list-benefit? false :white-list-verified? false
             :requires-id-number? true :id-number-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GEO" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "SPA eProcurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-black-list? false
             :claims-white-list-benefit? false :white-list-verified? false
    ...
    "eng-3" {:id "eng-3" :operator "Rioni Systems" :portal "SPA eProcurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "GEO" :status :intake}
    "eng-4" {:id "eng-4" :operator "Kolkheti Export LLC" :portal "SPA eProcurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-black-list? true
             :claims-white-list-benefit? false :white-list-verified? false
             ...
             :drafted? false :submitted? false
             :jurisdiction "GEO" :status :intake}
    "eng-5" {:id "eng-5" :operator "Mtkvari Logistics" :portal "SPA eProcurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-black-list? false
             :claims-white-list-benefit? false :white-list-verified? false
             :requires-id-number? true :id-number-verified? false
             :drafted? false :submitted? false
             :jurisdiction "GEO" :status :intake}
    "eng-6" {:id "eng-6" :operator "Tbilisi Civic Tech" :portal "SPA eProcurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-black-list? false
             :claims-white-list-benefit? true :white-list-verified? false
             :requires-id-number? true :id-number-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GEO" :status :intake}
    "eng-7" {:id "eng-7" :operator "Batumi Maritime Supply" :portal "SPA eProcurement"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :on-black-list? false
             :claims-white-list-benefit? true :white-list-verified? true
             :requires-id-number? true :id-number-verified? true
             :drafted? false :submitted? false
             :jurisdiction "GEO" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               on-black-list?
                               claims-white-list-benefit? white-list-verified?
                               requires-id-number? id-number-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? on-black-list?)                (assoc :engagement/on-black-list? on-black-list?)
    (some? claims-white-list-benefit?)    (assoc :engagement/claims-white-list-benefit? claims-white-list-benefit?)
    (some? white-list-verified?)          (assoc :engagement/white-list-verified? white-list-verified?)
    (some? requires-id-number?)           (assoc :engagement/requires-id-number? requires-id-number?)
    (some? id-number-verified?)           (assoc :engagement/id-number-verified? id-number-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/on-black-list?
   :engagement/claims-white-list-benefit? :engagement/white-list-verified?
   :engagement/requires-id-number? :engagement/id-number-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :on-black-list? (boolean (:engagement/on-black-list? m))
     :claims-white-list-benefit? (boolean (:engagement/claims-white-list-benefit? m))
     :white-list-verified? (boolean (:engagement/white-list-verified? m))
     :requires-id-number? (boolean (:engagement/requires-id-number? m))
     :id-number-verified? (boolean (:engagement/id-number-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-geo"})
